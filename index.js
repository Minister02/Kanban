...
const drop = event => {
	...
	updateTaskColumn(
		draggedTask.id,
		event.target.closest('.column').dataset.column_id
	);
...
btnAddTask.addEventListener('click', e => {
	...
	// Warunek sprawdzający czy w momencie kliknięcia w przycisk, formularz miał przypisany id istniejącego taska
	// Jeżeli nie miał, wtedy tworzony jest nowy
	// Jeżeli id zostało przypisane to wtedy aktualizowana jest tylko jego treść
	if (form.dataset.form_task_id === '') {
		...
		// Utworzenie nowego obiektu zadania
		const newObject = {
			id: taskId,
		...
		// Dodanie obiektu zadania do tablicy i sessionStorage
		allTasks.push(newObject);
		sessionStorage.setItem('tasks', JSON.stringify(allTasks));
	} else {
		// Pobranie istniejącego zadania z sessionStorage i zaktualizowanie jego danych
		const tasksFromSession = JSON.parse(sessionStorage.getItem('tasks'));
		const index = tasksFromSession.findIndex(
			task => task.id === form.dataset.form_task_id
		);
		const editingElement = tasksFromSession[index];
		const editingElementHtml = document.getElementById(
		...
		// Zaktualizowanie danych istniejącego zadania w interfejsie użytkownika
		...
		// Zaktualizowanie sessionStorage o nowe dane
...
const removeTask = taskId => {
	...
	const tasksFromSession = JSON.parse(sessionStorage.getItem('tasks'));
	const index = tasksFromSession.findIndex(
		task => task.id === taskId.dataset.id
	);
	if (index !== -1) {
		tasksFromSession.splice(index, 1);
...
// Funkcja odpowiedzialna za renderowanie zadań na stronie pobranych z sessionStorage
const renderTasks = () => {
	if (sessionStorage.getItem('tasks') !== null) {
		const tasksFromSession = JSON.parse(sessionStorage.getItem('tasks'));
		tasksFromSession.forEach(task => {
			let columnElement = document.querySelector(
				`[data-column_id="${task.columnId}"] .column-body`
			);
...
// Funkcja aktualizująca id kolumny po przeniesieniu zadania
const updateTaskColumn = (taskId, newColumnId) => {
	...
	// zaktualizowanie sessionStorage z nowymi informacjami o id kolumny dla zmienionego zadania, aby po odświeżeniu karty wszystkie zadania były w nowych kolumnach
	sessionStorage.setItem('tasks', JSON.stringify(tasksFromSession));
};
